  ...
  const handleSendBroadcast = async () => {
    ...
    try {
      ...
      if (data.success) {
        setResult(
          lang === 'bn'
            ? `ব্রডকাস্ট সফলভাবে পাঠানো শুরু হয়েছে (${data.totalRecipients} জন ইউজার)।`
            : `Broadcast queued successfully for ${data.totalRecipients} users!`
        );
        setMessage('');
      } else {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
      <div className="bg-white border border-[#e2e8f0] rounded-2xl p-6 max-w-lg w-full shadow-xl">
        <div className="flex items-center justify-between pb-3.5 border-b border-[#f1f5f9] mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-sm font-bold text-[#1e293b]">
              {lang === 'bn' ? 'সকল ইউজারের কাছে ব্রডকাস্ট নোটিশ' : 'Send Admin Broadcast'}
            </h3>
          </div>
        ...
        <p className="text-xs text-[#64748b] mb-3 leading-relaxed">
          {lang === 'bn'
            ? 'এই বার্তাটি বটের users.json এ থাকা সকল সক্রিয় ইউজারের টেলিগ্রাম চ্যাটে অ্যাডমিন নোটিশ হিসেবে পৌঁছাবে।'
        ...
        <div className="mb-4">
          <textarea
            ...
            placeholder={
              lang === 'bn'
                ? 'বার্তা লিখুন... (HTML ফরম্যাট যেমন <b>bold</b> বা <code>code</code> গ্রহণযোগ্য)'
        ...
        <div className="flex justify-end gap-2.5">
          ...
          >
            ...
            <span>{sending ? (lang === 'bn' ? 'পাঠানো হচ্ছে...' : 'Sending...') : (lang === 'bn' ? 'ব্রডকাস্ট পাঠান' : 'Send to All')}</span>
          </button>
        </div>
      </div>
    </div>
